...
import { ContextInfo } from "../hooks/useInterviewWS";
...
export function StreamingAnswer({ answer, isStreaming, error, status, contextInfo }: StreamingAnswerProps) {
  ...
  const isEmpty = !hasAnswer && !isStreaming && !error && !isSearching;
  const wordCount = answer.split(/\s+/).filter(Boolean).length;
  const minutesAloud = Math.ceil(wordCount / 130);
  ...
  return (
    <View className="mb-6">
        ...
        <View className="flex-row items-center gap-2">
          <View
            className={`w-1.5 h-1.5 rounded-full ${
              isStreaming ? "bg-streaming" : hasAnswer ? "bg-streaming-dim" : "bg-text-muted"
            }`}
          />
          ...
          {isSearching && (
            <Text style={{ fontFamily: "SpaceMono" }} className="text-accent-dim text-xs">⟳ searching</Text>
          )}
        </View>
      ...
      {/* Context badge — which resumes were used */}
      {contextInfo && contextInfo.sources.length > 0 && (
        <View className="mb-3 flex-row flex-wrap gap-1.5">
          {contextInfo.sources.map((src) => (
            <View key={src} className="bg-surface-elevated px-2.5 py-1 rounded-full border border-border">
              <Text style={{ fontFamily: "SpaceMono" }} className="text-text-muted text-xs">
                📄 {src}
              </Text>
            </View>
          ))}
          <View className="bg-surface-elevated px-2.5 py-1 rounded-full border border-border">
            <Text style={{ fontFamily: "SpaceMono" }} className="text-text-muted text-xs">
              {contextInfo.chunks} chunks
            </Text>
          </View>
        </View>
      )}
      ...
      {/* Answer box */}
      <View
        className={`rounded-xl border min-h-32 ${isStreaming ? "border-streaming-dim" : "border-border"}`}
      ...
      >
        ...
        {isSearching && !hasAnswer && (
          <View className="flex-1 items-center justify-center p-8 min-h-32">
            <Text style={{ fontFamily: "SpaceMono" }} className="text-accent-dim text-xs text-center">
              Searching resume index...
            </Text>
          </View>
        )}
        ...
        {error && (
          <View className="p-5">
            <Text className="text-danger text-sm font-semibold mb-2">Error</Text>
          ...
          >
            ...
            {!isStreaming && hasAnswer && (
              <View className="mt-4 pt-4 border-t border-border">
                <Text style={{ fontFamily: "SpaceMono" }} className="text-text-muted text-xs">
                  {wordCount} words · ~{minutesAloud} min aloud
                </Text>
              </View>
            )}
          </ScrollView>
